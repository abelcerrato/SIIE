...
const EditRole = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const roleRes = await axios.get(`${process.env.REACT_APP_API_URL}/permiso/${id}`);
        ...
        if (permissionsArray.length > 0) {
          ...
          const permisosPorModulo = Array.from(modulosMap.values()).map((modulo) => {
            const permisosDelModulo = permissionsArray.filter(
              (p) => p.idmodulo === modulo.id
            );

            const consultar = permisosDelModulo.some(p => p.consultar);
            const insertar = permisosDelModulo.some(p => p.insertar);
            const actualizar = permisosDelModulo.some(p => p.actualizar);
  ...
  const handleSave = async () => {
    try {
      ...
      Swal.fire({
        title: "Error",
        text: error.response?.data?.message || "Ocurrió un error al actualizar el rol",
        icon: "error",
        confirmButtonColor: "#88CFE0",
      });
    }
                  ...
                  <TableRow key={permiso.idmodulo} hover>
                    ...
                    <TableCell align="center">
                      <Checkbox
                        ...
                        onChange={() => {
                          handlePermissionChange(permiso.idmodulo, "actualizar");
