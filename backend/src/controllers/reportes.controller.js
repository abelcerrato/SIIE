...
// Controlador para mostrar la matricula del año 2024 de DES
export const getDES_matricula_2024C = async (req, res) => {
  try {
    const des = await getDES_matricula_2024M();
    res.json(des);
  } catch (error) {
    console.error("Error al obtener la matricula del año 2024 de DES:", error);
    res.status(500).json({ error: "Error interno del servidor" });
  }
};
